...
- **Идентификация лидов**: по external_id, email или phone
- **Взвешенное распределение**: случайный выбор с учетом весов
- **Контроль нагрузки**: операторы не получают новые обращения при превышении лимита
- **Гибкая настройка**: веса операторов настраиваются для каждого источника отдельно
- **Обработка ошибок**: обращения без доступных операторов создаются без назначения
